...
void init_colors()
{
	...
	b2=getenv("HOME");
	for (i=0;i<strlen(b2);i++) b3[i]=b2[i];
	b3[i++]='/';
	b3[i++]='.';
	b3[i++]='d';
	b3[i++]='h';
...
char *input2(WINDOW *parent_window,int y,int x,int len,const char *text,int max,int special,int firstch)
{
	int cursorpos=0;
	int offset=0;
	int l=0;
...
//		copywin(fulltext,parent_window,0,offset,y,x,y,x+len,1);
		...
		for (i=0;i<max;i++)
		{
			if (i>=offset && i<=offset+len) waddch(parent_window,textfield[i]);
		}
...
//	mvwaddch(parent_window,1,1,'=');
	...
	for (i=0;i<max;i++)
	{
		if (i>=0 && i<=len) mvwaddch(parent_window,y,x+i,textfield[i]);
	}
	wrefresh(parent_window);
	mvwaddch(parent_window,0,0,' ');
	wmove(parent_window,0,0);
...
char *input(WINDOW *parent_window,int y,int x,int len,const char *text,int max)
{
