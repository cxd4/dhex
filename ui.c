...
tInt8 gotomask(tOutput* output,tUInt64* cursorpos)
{
	...
	drawcenterframe(output,3,42,"Goto...");
	setcolor(output,COLOR_BRACKETS);
	mvwprintw(output->win,offsy+3,offsx+6,"[                 ]");
	clearMenu(&Menu1);
	...
	while (1)
	{
		mvwprintw(output->win,offsy+3,offsx+7,"%17llx",newcursorpos);
		selected=MenuInteract(output,&Menu1,LINES/2-2,COLS/2-21);
		if (selected==itemnums[0]) {newcursorpos=actcursorpos;hexinput(output,offsy+3,offsx+6,&newcursorpos,17);}
